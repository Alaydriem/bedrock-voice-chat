use serde::{Deserialize, Serialize};
use std::time::{SystemTime, UNIX_EPOCH};

use super::quic_network_packet_data::QuicNetworkPacketData;
use crate::structs::control::QueryState;

#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct QueryStatePacket {
    pub state: QueryState,
    pub occurred_at_ms: u64,
}

impl QueryStatePacket {
    pub fn new(state: QueryState) -> Self {
        Self {
            state,
            occurred_at_ms: SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap_or_default()
                .as_millis() as u64,
        }
    }
}

impl TryFrom<QuicNetworkPacketData> for QueryStatePacket {
    type Error = ();

    fn try_from(value: QuicNetworkPacketData) -> Result<Self, Self::Error> {
        match value {
            QuicNetworkPacketData::QueryState(c) => Ok(c),
            _ => Err(()),
        }
    }
}
