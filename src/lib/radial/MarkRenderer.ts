 ...
 *
 * Ported from client/src/radial/core/mark/MarkRenderer.ts.
 *
 * This is the one routine. The header logo, every level meter, the support
 * meters in the compatibility panel and the shape at the centre of the ring are
 * all this function at different amplitudes and cell sizes — which is why they
 * read as the same object rather than as a logo plus some bar charts.
 *
...
export class MarkRenderer {
  ...
  static extent(col: number, amplitude: number): readonly [number, number] {
    ...
    const { MID } = MarkData;
    return [Math.round(MID - (MID - top) * amplitude), Math.round(MID + (bottom - MID) * amplitude)];
  }
