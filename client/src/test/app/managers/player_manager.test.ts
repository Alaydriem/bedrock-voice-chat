...
/** The persisted store, in memory. Nothing here exercises a write. */
function store() {
    ...
    return {
        ...
        save: async () => {},
    } as never as import("@tauri-apps/plugin-store").Store;
}
...
describe("PlayerManager identity", () => {
    ...
    it("recognises itself when it was told a bare gamertag", () => {
        const manager = new PlayerManager(store(), "Alaydriem", "minecraft");
    ...
    it("composes a gamertag set after construction", () => {
        const manager = new PlayerManager(store(), "", "hytale");
    ...
    it("holds no identity before it has been told who this is", () => {
        const manager = new PlayerManager(store(), "", "minecraft");
