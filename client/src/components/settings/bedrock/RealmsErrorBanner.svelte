...
{#if $connectionError}
    <div class="card p-4 border-l-4 border-error">
        <div class="flex items-start gap-3">
            <svg xmlns="http://www.w3.org/2000/svg" class="size-5 mt-0.5 shrink-0 text-error" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex-1">
                <p class="text-sm font-medium text-slate-800 dark:text-navy-100">
                    ...
                    {:else if $connectionError.kind === "raknet"}
                        RakNet error
                    {:else if $connectionError.kind === "auth"}
                        Authentication error
                    {:else}
                        Realms connection error
                    {/if}
                </p>
                <p class="mt-1 text-sm text-slate-600 dark:text-navy-200">{$connectionError.suggestion}</p>
                <p class="mt-1 text-xs text-slate-400 dark:text-navy-300 break-all">{$connectionError.message}</p>
