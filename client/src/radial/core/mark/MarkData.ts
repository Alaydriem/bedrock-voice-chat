 ...
 * The mark.
 *
 * `static/images/app-logo-transparent.svg` is not a badge. It is 137 blocks on a
 * 23x13 grid where every column is one contiguous vertical span — a waveform
 * envelope running violet to red across the spectrum. That compresses to the 23
 * triplets below, and one drawing routine renders them linearly (a header, a
 * level meter), radially (the ring), and at any amplitude.
 *
 * Geometry is verified against `static/images/cells.json`, the per-cell export of
 * the SVG: all 23 spans agree exactly. Four columns' sampled hex differ from the
 * values below by at most 2/255 in one channel, which is antialiasing in the
 * export rather than a discrepancy; the values below are canonical because the
 * palette tokens are defined against them.
 */
