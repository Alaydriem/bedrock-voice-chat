...
impl McOauthWindow {
    pub async fn open(app_handle: tauri::AppHandle) -> Result<String, String> {
        ...
        let mut builder =
            WebviewWindowBuilder::new(&app_handle, &label, tauri::WebviewUrl::External(url))
        ...
        rx.await
            .map_err(|_| "OAuth window closed without completing".to_string())
    }
