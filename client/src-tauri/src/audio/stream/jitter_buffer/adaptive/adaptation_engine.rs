use super::{AdaptiveBufferState, CongestionLevel, NetworkQuality};
...
impl AdaptationEngine {
    ...
    pub fn assess_network_conditions(&mut self, metrics: &MetricsCollector) -> NetworkQuality {
        ...
        self.network_quality =
            NetworkQuality::from_metrics(loss_rate, jitter, metrics.network_metrics.rtt_variance);
    ...
    pub fn calculate_target_capacity(&self) -> usize {
        ...
        self.state
            .calculate_target_capacity(self.base_capacity, combined_multiplier)
    }
    ...
    pub fn should_adjust_buffer(&self, metrics: &MetricsCollector) -> bool {
        ...
        let capacity_diff_ratio = (target_capacity as f64 - self.state.current_capacity as f64)
            .abs()
