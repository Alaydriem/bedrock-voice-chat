...
impl JitterBufferSource {
    ...
    pub fn new_with_activity(
        ...
        source.metrics_collector.record_packet_arrival(
            initial_packet.timestamp, 
    ...
    fn drain_incoming(&mut self) {
        ...
        if let Some(new_capacity) = self.adaptation_engine.adjust_buffer_if_needed(&self.metrics_collector) {
            self.metrics_collector.record_adaptation(self.last_accepted_timestamp);
    ...
    fn is_packet_acceptable(&self, packet_timestamp: u64) -> bool {
        ...
        self.adaptation_engine.is_timestamp_acceptable(packet_timestamp, self.last_accepted_timestamp)
    }
        ...
        if let Some(packet) = self.packet_ring.pop_front() {
            match self.audio_processor.decode_opus(&packet.data) {
                Ok(frames_written) => {
                    ...
                    self.adaptation_engine.assess_network_conditions(&self.metrics_collector);
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(sample) = self.audio_processor.next_sample() {
            ...
            if self.audio_processor.frame_sample_countdown == 0 {
                self.last_output_ts_ms = self.last_output_ts_ms.saturating_add(20); // FRAME_MS
            }
            return Some(sample);
        }
