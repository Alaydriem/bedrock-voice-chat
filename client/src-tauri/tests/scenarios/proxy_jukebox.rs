...
use crate::harness::jukebox_fixture::{BM_NOTES, JukeboxFixture};
...
async fn proxy_jukebox_both_in_range_hear_then_eject_stops() {
    for v in ProtocolMatrix::last_two() {
        ...
        for _ in 0..5 {
            w.upstream
                .drive_position("Alice", NEAR.0, NEAR.1, NEAR.2)
                .await;
            w.upstream
                .drive_position("Bob", NEAR.0, NEAR.1, NEAR.2)
                .await;
        ...
        let has_a = NoteEnergy::all_present(&mono_a, &BM_NOTES);
        let has_b = NoteEnergy::all_present(&mono_b, &BM_NOTES);
        ...
        assert!(
            a_fq > 0 && b_fq > 0,
            "[{v}] C1: both receive frames from QUIC mid-play"
        );
...
async fn proxy_jukebox_in_range_hears_out_silent() {
    for v in ProtocolMatrix::last_two() {
        ...
        let fixture_dir = tempfile::tempdir().expect("fixture dir");
        let wav = JukeboxFixture::bm_wav(fixture_dir.path(), "track", 2);
        let (audio_id, duration_ms) = w
            .proc("Alice")
            .upload_audio(wav.to_str().unwrap(), "minecraft", Duration::from_secs(30))
            .expect("Alice uploads track");
...
async fn proxy_jukebox_natural_end_returns_to_silence() {
    for v in ProtocolMatrix::last_two() {
        ...
        let fixture_dir = tempfile::tempdir().expect("fixture dir");
        let wav = JukeboxFixture::bm_wav(fixture_dir.path(), "track", 1);
        let (audio_id, duration_ms) = w
            .proc("Alice")
        ...
        eprintln!(
            ...
            NoteEnergy::all_present(&mono_play, &BM_NOTES),
        ...
        let cap_after = w
            .proc("Alice")
            .collect_captured(Duration::from_millis(2_000));
...
async fn proxy_jukebox_explicit_eject_returns_to_silence() {
    for v in ProtocolMatrix::last_two() {
        ...
        eprintln!(
            ...
            NoteEnergy::all_present(&mono_mid, &BM_NOTES),
            Signal::rms(&mono_mid),
        );
        assert!(
            NoteEnergy::all_present(&mono_mid, &BM_NOTES),
        ...
        let cap_after = w
            .proc("Alice")
            .collect_captured(Duration::from_millis(1_000));
        let rms_after = Signal::rms(&Signal::to_mono(&cap_after));
...
async fn proxy_jukebox_ended_with_nobody_then_late_arrival_silent() {
    for v in ProtocolMatrix::last_two() {
        ...
        for _ in 0..5 {
            w.upstream
                .drive_position("Alice", FAR.0, FAR.1, FAR.2)
                .await;
            w.upstream
                .drive_position("Bob", FAR.0 + 10.0, FAR.1, FAR.2)
                .await;
            tokio::time::sleep(Duration::from_millis(120)).await;
        }
