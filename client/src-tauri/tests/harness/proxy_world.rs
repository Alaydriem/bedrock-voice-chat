...
use bedrock_client::{BedrockClient, ClientConnection};
...
impl ProxyWorld {
    pub async fn boot(version: ProtocolVersion, names: &[&str]) -> Self {
        ...
        for &name in names {
            ...
            proc.await_connected(Duration::from_secs(30)).expect("voice connect");
            ...
            assert_eq!(accepted, name, "upstream connection identity must match actor");

            upstream.start_game(name).await;

            players.insert(name.to_string(), ProxyPlayer { proc, _downstream: downstream });
        }

        Self { server, upstream, players, version, _data_dir: data_dir }
    }
