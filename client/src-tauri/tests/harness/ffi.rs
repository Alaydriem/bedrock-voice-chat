...
impl ServerLibrary {
    ...
    pub fn load(path: &std::path::Path) -> Result<Arc<Self>, String> {
        ...
        unsafe {
            ...
            let version = Self::sym::<unsafe extern "C" fn() -> *const c_char>(&lib, b"bvc_version")?;
            let create =
                Self::sym::<unsafe extern "C" fn(*const c_char) -> RuntimeHandlePtr>(&lib, b"bvc_server_create")?;
            let start = Self::sym::<unsafe extern "C" fn(RuntimeHandlePtr) -> c_int>(&lib, b"bvc_server_start")?;
            let stop = Self::sym::<unsafe extern "C" fn(RuntimeHandlePtr) -> c_int>(&lib, b"bvc_server_stop")?;
            let destroy = Self::sym::<unsafe extern "C" fn(RuntimeHandlePtr) -> c_int>(&lib, b"bvc_server_destroy")?;
            let last_error = Self::sym::<unsafe extern "C" fn() -> *const c_char>(&lib, b"bvc_get_last_error")?;
            let update_positions = Self::sym::<unsafe extern "C" fn(RuntimeHandlePtr, *const c_char) -> c_int>(
                &lib,
                b"bvc_update_positions",
            )?;
            let audio_play = Self::sym::<unsafe extern "C" fn(RuntimeHandlePtr, *const c_char) -> *mut c_char>(
                &lib,
                b"bvc_audio_play",
            )?;
            let audio_stop =
                Self::sym::<unsafe extern "C" fn(RuntimeHandlePtr, *const c_char) -> c_int>(&lib, b"bvc_audio_stop")?;
            let provision_login_code = Self::sym::<
                unsafe extern "C" fn(RuntimeHandlePtr, *const c_char, *const c_char, u32) -> *mut c_char,
            >(&lib, b"bvc_provision_login_code")?;
            let free_string = Self::sym::<unsafe extern "C" fn(*mut c_char)>(&lib, b"bvc_free_string")?;
