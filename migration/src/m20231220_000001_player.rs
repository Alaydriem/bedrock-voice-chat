            ...
            ).await
        {
            Ok(_result) => {
                let stmt = Statement::from_string(
                    ...
                    "ALTER TABLE Player ALTER COLUMN updated_at SET DEFAULT UNIX_TIMESTAMP();".to_owned()
                );
                return manager
                    .get_connection()
                    .execute(stmt).await
            ...
            Err(_) => Err(DbErr::Migration("Unable to migrate `Player` table.".to_owned())),
        }
    }

    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        return manager.drop_table(Table::drop().table(Player::Table).to_owned()).await;
    }
}
