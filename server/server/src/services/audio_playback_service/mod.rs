...
/// Service managing audio playback sessions.
/// Uses a moka cache with per-entry TTL matching each file's duration.
...
impl AudioPlaybackService {
    ...
    /// Start audio playback at the given coordinates.
    pub async fn start_playback<C: ConnectionTrait>(
        ...
        // Load audio file metadata from DB
        let audio_file = audio_file::Entity::find_by_id(request.audio_file_id.clone())
        ...
        tracing::info!(
            event_id = %event_id,
            file_id = %request.audio_file_id,
            duration_ms = duration_ms,
            frames = frames.len(),
            "Starting audio playback"
        );

        // Build synthetic player at block coordinates
        let synthetic_player = PlayerEnum::Minecraft(MinecraftPlayer {
            name: format!("jukebox::{}", event_id),
            coordinates: request.coordinates,
            orientation: Orientation { x: 0.0, y: 0.0 },
            dimension: request.dimension,
            deafen: false,
            spectator: false,
            world_uuid: Some(request.world_uuid),
        });
        ...
        // Construct and spawn PlaybackTask
    ...
    /// Stop all active playback sessions (server shutdown).
    pub fn stop_all(&self) {
        self.parent_token.cancel();
    }

    /// Check if a specific audio file is currently being played.
