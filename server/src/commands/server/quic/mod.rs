...
type AsyncProducerBuffer = Sender<QuicNetworkPacket>;
type AsyncConsumerBuffer = Receiver<QuicNetworkPacket>;
type MutexMapProducer = Arc<Mutex<Sender<QuicNetworkPacket>>>;
...
pub(crate) async fn get_task(
    ...
    let (main_producer, main_consumer) =
        flume::bounded::<QuicNetworkPacket>(MAIN_RINGBUFFER_CAPACITY);
    ...
    tasks.push(
        tokio::spawn(async move {
            ...
            while let Some(mut connection) = server.accept().await {
                ...
                let (producer, consumer) = flume::bounded::<QuicNetworkPacket>(
                ...
                _ = mutex_map
                    .lock_arc().await
                    .insert(connection_id, Arc::new(Mutex::new(producer)));
                    ...
                    Ok(stream) =>
                        match stream {
                            Some(stream) => {
                                ...
                                let receiver_mutex_map = mutex_map.clone();
                                tokio::spawn(async move {
                                    let mutex_map = receiver_mutex_map.clone();
                                    ...
                                    loop {
                                        // This sets an idle timer on the loop so we can check if we received a shutdown signal or if we received a disconnect signal.
                                        let data = tokio::time::timeout(
                                            Duration::from_millis(20),
                                            async {
                                                receive_stream.receive().await
                                            }
                                        ).await;
                                            ...
                                            disconnected.load(Ordering::Relaxed)
                                        {
                                            ...
                                            tracing::info!(
                                                "Disconnected receiving stream for {}",
                                                connection_id
                                            );
                                            break;
                                        }
                                            ...
                                            packet.len() >= packet_len + 13
                                        {
                                            ...
                                            match QuicNetworkPacket::from_vec(&packet_to_process) {
                                                Ok(packet) => {
                                                    ...
                                                    _ = main_producer_mux.send_async(packet).await;
                                                }
                                                Err(_) => {
                                                    continue;
                                                }
                                    ...
                                    _ = mutex_map.lock_arc().await.remove(&connection_id);
                                    tracing::info!("Receving stream for {} ended.", &connection_id);
                                    return;
                                });
                                ...
                                let sender_mutex_map = mutex_map.clone();
                                tokio::spawn(async move {
                                    let mutex_map = sender_mutex_map.clone();
                                    ...
                                    disconnected.store(true, Ordering::Relaxed);
                                    _ = mutex_map.lock_arc().await.remove(&connection_id);
                                    tracing::info!("Sending stream for {} ended.", &connection_id);
                                    return;
                                });
                            }
                            None => {}
    ...
    tasks.push(
        tokio::spawn(async move {
            ...
            while let packet = main_consumer_mux.recv_async().await {
                ...
                match packet {
                    Ok(packet) => {
                        for (_, producer) in mutex_map.lock_arc().await.clone().into_iter() {
                            let producer = producer.lock_arc().await;
                            _ = producer.send_async(packet.clone()).await;
                        }
                    }
                    Err(_) => {}
                }
